...
render=function(){clearInterval(wordTimer);clearInterval(typeTimer);originalRender();const isHome=location.pathname==='/' ;document.body.classList.toggle('is-original-home',isHome);if(isHome){document.querySelector('.video-cover').onclick=()=>{document.querySelector('.original-player').innerHTML='<iframe src="https://www.youtube.com/embed/gCq7XP2yFVA?autoplay=1&rel=0" title="Wasib Imdad — introduction film" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>';document.querySelector('.original-video').classList.add('is-playing');};if(!matchMedia('(prefers-reduced-motion: reduce)').matches){let i=0;const words=['Brands','Stories','Narratives'];const typeNext=()=>{const el=document.querySelector('#creating-word');if(!el)return;const word=words[++i%words.length];let n=0;el.textContent='';clearInterval(typeTimer);typeTimer=setInterval(()=>{el.textContent=word.slice(0,++n);if(n>=word.length){clearInterval(typeTimer);wordTimer=setTimeout(typeNext,2600)}},95)};wordTimer=setTimeout(typeNext,3000)}}};
