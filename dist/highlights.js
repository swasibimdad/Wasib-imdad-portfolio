(() => {
  ...
  render=function(){
    ...
    section.innerHTML=`<div class="highlights-track">${statements.map((text,i)=>`<article class="highlight-slide" aria-label="Highlight ${i+1} of 5"><img src="/assets/${['highlights-stage.jpg','highlights-2.png','highlights-3.png','highlights-4.png','highlights-5.png'][i]}" alt="" loading="lazy"><div class="highlight-copy"><span>Highlights · ${String(i+1).padStart(2,'0')}</span><p>${text}</p></div></article>`).join('')}</div><div class="highlights-controls"><div class="highlights-dots" aria-label="Choose highlight">${statements.map((_,i)=>`<button aria-label="Show highlight ${i+1}" aria-current="${i===0}"></button>`).join('')}</div><button class="highlights-scroll" aria-label="Scroll to continue"><span aria-hidden="true">⌄</span></button></div>`;
